...
# called by dracut
check() {
    require_binaries -s curl || return 1
    return 255
}
...
install() {
    ...
    if ! [[ $_found ]] && [[ $_nssckbi ]]; then
        ...
        for _dir in $libdirs; do
            ...
            grep -z -o --binary-files=text '/[[:alpha:]][[:print:]]*' "${_dir}"/libnssckbi.so \
                | while read -r -d '' _p11roots || [[ $_p11roots ]]; do
                    ...
                    for _p11root in "${_p11roots[@]}"; do
                        ...
                        if ! $DRACUT_CP -L -t "${initdir}/${_p11root}" "${_p11root}"/*; then
                            dwarn "Couldn't install from p11-kit trust dir '${_p11root}'; HTTPS might not work."
                        fi
                    done
                done
        done
