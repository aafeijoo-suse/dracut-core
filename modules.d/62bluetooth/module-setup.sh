...
check() {
    ...
    if [[ $hostonly ]]; then
        ...
        [ -d "/sys/class/bluetooth" ] && grep -qiE -e 'Class=0x[0-9a-f]{3}5[4c]0' -e 'Appearance=0x03c1' /var/lib/bluetooth/*/*/info 2> /dev/null && return 0
    fi
...
install() {
    ...
    if [[ $hostonly ]]; then
        var_lib_files=(/var/lib/bluetooth/**)
